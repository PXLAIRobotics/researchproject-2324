...
class Camera():
    ...
    def __init__(self, params):
        try:
            ...
            self.stream = requests.get(mjpeg_url, auth=HTTPDigestAuth(username, password), stream=True)

            # Check if the request was successful (HTTP status code 200)
            if self.stream.status_code == 200:
                print("Ready for action!")
            else:
                print("Failed to retrieve MJPEG stream. HTTP status code:", stream.status_code)
    ...
    def show(self):
        ...
        for chunk in self.stream.iter_content(chunk_size=1024):
            # Process the image bytes here as needed
            bytes += chunk
...
if __name__ == '__main__':
    params = {
        "url": "192.168.1.63/axis-cgi",
        'name': "root",
        'password': "7v5sgKiAlO9M",
